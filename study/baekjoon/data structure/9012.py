...
이라고 부른다. 한 쌍의 괄호 기호로 된 “( )” 문자열은 기본 VPS 이라고 부른다. 
...
def solution(n, array):
    # 아이디어: 스택 자료구조를 활용하는 대표적인 예제
    result = ['YES' for _ in range(n)]
    for i in range(n):
        stack = []
        # 여기에서 i번째 문자열이 VPS인지 체크
        for j in range((len(array[i]))):
            if array[i][j]=='(':
                ...
                if len(stack) > 0:
                    ...
                    result[i]='NO' # pop 할 '('이  없는데 ')' 등장
                    break  
        if len(stack) > 0:
            result[i]='NO' # '('이 ')'보다 더 많은 경우
    return '\n'.join(result)
